...
async function loadAndOpenTrailer(movieId: string) {
  try {
    ...
    const trailers = movieTrailersData.results.filter((video: any) => video.type === 'Trailer');
...
function openTrailerInInfoAboutMovie(movieId: string) {
...
function playTrailerClick(movieId: string) {
  const playTrailer = document.getElementById('additional-video-trailer');
